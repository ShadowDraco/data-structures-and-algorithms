...
const alphabetizeBetter = (arr) => {
  return arr.sort((a, b) => {
    console.log(a, b);
    a = a.toLowerCase().charCodeAt(a[0]);
    b = b.toLowerCase().charCodeAt(b[0]);
...
const sortNumbersByLength = (arr) => {
  return arr.sort((a, b) => {
    ...
    if (newA.length > newB.length) return 1;
    if (newA.length < newB.length) return -1;
    if (newA.length === newB.length) return 0;
  });
};
...
const sortPeople = (arr) => {
  return arr.sort((a, b) => {
    let newA = a.lastName.charCodeAt(0);
    let newB = b.lastName.charCodeAt(0);
...
const sortPeopleBetter = (arr) => {
  return arr.sort((a, b) => {
    ...
    for (let char in a.lastName) {
      aLastValue += a.lastName.charCodeAt(char);
    }
    for (let char in b.lastName) {
      bLastValue += b.lastName.charCodeAt(char);
    }

    if (aLastValue > bLastValue) return 1;
    if (aLastValue < bLastValue) return -1;

    if (aLastValue === bLastValue) {
      for (let char in a.firstName) {
        aFirstValue += a.firstName.charCodeAt(char);
      }
      for (let char in b.firstName) {
        bFirstValue += b.firstName.charCodeAt(char);
      }

      if (aFirstValue > bFirstValue) return 1;
      if (aFirstValue < bFirstValue) return -1;
...
const sortMeetingsByDay = (arr) => {
  ...
  return arr.sort((a, b) => {
    ...
    if (days[a.dayOfWeek] < days[b.dayOfWeek]) return -1;
    if (days[a.dayOfWeek] === days[b.dayOfWeek]) return 0;
  });
};
