...
const sortByChildren = (charArray) => {
  return charArray.sort((a, b) => {
    ...
    if (a.children.length === b.children.length) {
      if (a.house.charCodeAt(0) > b.house.charCodeAt(0)) return 1;
      if (a.house.charCodeAt(0) < b.house.charCodeAt(0)) return -1;
      if (a.house.charCodeAt(0) === b.house.charCodeAt(0)) return 0;
    }
  });
};
...
const citiesAtoJ = (arr) => {
  let filtered = [];
  filtered = arr.filter((city) => {
    return city.match(/(^[A-J]\w+)/) ? true : false;
  });
  return filtered;
};
