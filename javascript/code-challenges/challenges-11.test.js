...
const totalSum = (input) => {
  return input.flat(1).reduce((acc, value) => {
    return acc + value;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let numbers = input.map((arr) => {
    let filtered = arr.filter(
      (number) => typeof number === "number" && number % 5 === 0
    );
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach((person) => {
    if (person.gender === "male" || person.gender === "female") {
      malesAndFemales.push(person.name);
    }
  });

  return malesAndFemales.join(" and ");
};
...
let findShortest = (data) => {
  let firstHeight = parseInt(data[0].height);
  let shortestPerson;
  let shortest = data.reduce((acc, person) => {
    return parseInt(person.height) < acc ? parseInt(person.height) : acc;
  }, firstHeight);
  data.forEach((person) => {
    if (parseInt(person.height) === shortest) {
      shortestPerson = person.name;
      return;
    }
  });
