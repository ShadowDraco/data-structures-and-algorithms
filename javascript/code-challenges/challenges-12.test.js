...
const validateWord = (word) => {
  return typeof word === "string" ? word.match(/^\D{5,10}$/) : false;
};
...
const hasNumber = (string) => {
  return string.toString().match(/[a-zA-Z]+\d+/g);
};
...
const validateEmail = (email) => {
  return email.match(/^\S(?:(?:\w+\.\w+)|\w+)\@\w+\.[net|org|com]+$/);
};
...
const validatePhoneNumber = (phoneNumber) => {
  return phoneNumber.match(/^((\(\d{3}\))|(\d{3}))[- ]?\d{3}[- ]?\d{4}$/);
};
...
(?:
  ...
  ) // group 555 or (555) followed by ' ' or - or 555 followed by ' ' or -
      | // or group 5555555555 or 555 5555555
      (?:
        (?:\d{10}))
      |(?:\d{3} \d{7})
)
